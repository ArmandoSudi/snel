...
class API {
  ...
  Stream<QuerySnapshot> getAgentByPhone(String phone) {
    return _firebase.collection('clients')
  ...
  Future<Client> getClient(String phone)  async {
    return await _firebase.collection('clients')
        .where("phone_number", isEqualTo: phone)
        .snapshots().first.then((value) => Client.fromDocument(value.docs.first));
  }

  // Get a stream of invoices where compteur_id is equal to the given id
  Stream<QuerySnapshot> getInvoicesByCompteurId(String counterId) {
    return _firebase.collection('invoices')
  ...
  Future<List<Counter>> getCounterByAgentId(String agentId) async {
    try {
      return await _firebase.collection('counters')
          .where("agent_id", isEqualTo: agentId)
          .get()
          .then((querySnapshot) => querySnapshot.docs.map((e) =>
          Counter.fromJson(e.data()! as Map<String, dynamic>)).toList());
    } catch (e) {
      print("getUserData::: ${e.toString()}");
      return [];
    }
  ...
  Stream<QuerySnapshot> getPaymentsByCompteurId(String counterId) {
    return _firebase.collection('payments')
        .where("counter", isEqualTo: counterId)
        .snapshots();
  }
  ...
  Future<DocumentReference> createCounter(Counter counter) {
    ...
    Future<void> setPaidInvoice(Invoice invoice) {
      ...
      return _firebase.collection('invoices').doc(invoice.id).update(
          {'is_paid': true});
    }
